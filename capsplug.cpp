...
CapsProc cpr[]= {
    ...
    "CAPSGetImageType", NULL,
    "CAPSGetImageTypeMemory", NULL,
    "CAPSGetDebugRequest", NULL,
	NULL, NULL
};
...
SDWORD CAPSGetInfo(PVOID pinfo, SDWORD id, UDWORD cylinder, UDWORD head, UDWORD inftype, UDWORD infid) {
    SDWORD res = cpr[13].proc ? CAPSHOOKN(cpr[13].proc)(id, cylinder, head, inftype, infid) : imgeUnsupported;

    return res;
}

SDWORD CAPSSetRevolution(SDWORD id, UDWORD value) {
    SDWORD res = cpr[14].proc ? CAPSHOOKN(cpr[14].proc)(id, value) : imgeUnsupported;

    return res;
}

SDWORD CAPSGetImageType(PCHAR name) {
    SDWORD res = cpr[15].proc ? CAPSHOOKN(cpr[15].proc)(name) : imgeUnsupported;

    return res;
}

SDWORD CAPSGetImageTypeMemory(PUBYTE buffer, UDWORD length) {
    SDWORD res = cpr[16].proc ? CAPSHOOKN(cpr[16].proc)(buffer, length) : imgeUnsupported;
